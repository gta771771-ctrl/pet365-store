...
router.get('/my-enrollments', authMiddleware, (req, res) => {
  try {
    const discount = db.all("SELECT me.*, dp.name as plan_name, dp.discount_percent, dp.monthly_fee, dp.features, dp.max_pets FROM member_discount_enrollments me JOIN discount_plans dp ON dp.id = me.plan_id WHERE me.user_id = ? ORDER BY me.created_at DESC", [req.user.id]);
...
router.post('/enroll-discount', authMiddleware, (req, res) => {
  try {
    const { plan_id, pet_ids } = req.body;
    ...
    if (existing) return res.status(400).json({ success: false, message: 'Already enrolled in this plan' });
    // Validate pets
    let pets = [];
    if (pet_ids && Array.isArray(pet_ids) && pet_ids.length > 0) {
      const userPetIds = db.all("SELECT id FROM pets WHERE user_id = ?", [req.user.id]).map(p => p.id);
      pet_ids.forEach(pid => { if (userPetIds.includes(pid)) pets.push(pid); });
    }
    if (pets.length === 0) {
      return res.status(400).json({ success: false, message: 'At least one pet is required' });
    }
    const petsJson = JSON.stringify(pets);
    const result = db.run("INSERT INTO member_discount_enrollments (user_id, plan_id, status, end_date, pets) VALUES (?, ?, 'active', datetime('now', '+1 year'), ?)",
      [req.user.id, plan_id, petsJson]);
    const enrollment = db.get("SELECT * FROM member_discount_enrollments WHERE id = ?", [result.lastInsertRowid]);
    res.json({ success: true, data: enrollment, message: 'Enrolled! Pet(s) are now covered.' });
  } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
router.post('/enroll-wellness', authMiddleware, (req, res) => {
  try {
    ...
    const now = new Date();
    const petsJson = JSON.stringify(pets);
    const result = db.run("INSERT INTO member_wellness_enrollments (user_id, plan_id, status, annual_year, end_date, pets) VALUES (?, ?, 'active', ?, datetime('now', '+1 year'), ?)",
      [req.user.id, plan_id, now.getFullYear(), petsJson]);
    const enrollment = db.get("SELECT * FROM member_wellness_enrollments WHERE id = ?", [result.lastInsertRowid]);
    res.json({ success: true, data: enrollment, message: 'Enrolled! Pet(s) are now covered.' });
  } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
router.post('/vet-transaction', authMiddleware, (req, res) => {
  try {
    ...
    if (enrollment) {
      // Check if pet is covered
      let covered = true;
      if (enrollment.pets && pet_id) {
        try {
          const coveredPets = JSON.parse(enrollment.pets);
          covered = coveredPets.includes(pet_id);
        } catch(e) {}
      }
      if (covered) {
        discountAmount = finalAmount * (enrollment.discount_percent / 100);
...
router.get('/vet-transactions', authMiddleware, (req, res) => {
  try { res.json({ success: true, data: db.all("SELECT vt.*, p.name as pet_name, c.name as clinic_name FROM vet_transactions vt LEFT JOIN pets p ON p.id = vt.pet_id LEFT JOIN vet_clinics c ON c.id = vt.clinic_id WHERE vt.user_id = ? ORDER BY vt.transaction_date DESC", [req.user.id]) }); }
  catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
router.post('/reimbursement', authMiddleware, (req, res) => {
  try {
    ...
    const remaining = enrollment.max_reimbursement - (enrollment.reimbursement_used || 0);
    if (reimbAmount > remaining) reimbAmount = Math.max(0, remaining);
    ...
    res.json({ success: true, data: db.get("SELECT * FROM reimbursement_claims WHERE id = ?", [result.lastInsertRowid]), message: 'Claim submitted! Estimated reimbursement: $' + reimbAmount.toFixed(2) });
  } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});

router.get('/reimbursements', authMiddleware, (req, res) => {
  try { res.json({ success: true, data: db.all("SELECT rc.*, p.name as pet_name FROM reimbursement_claims rc LEFT JOIN pets p ON p.id = rc.pet_id WHERE rc.user_id = ? ORDER BY rc.created_at DESC", [req.user.id]) }); }
  catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
